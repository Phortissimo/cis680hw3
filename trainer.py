...
class Trainer(object):
  def __init__(self, config, data_loader, label_loader, loc_loader, mask_loader, test_data_loader, test_label_loader, test_loc_loader, test_mask_loader):
    self.config = config
    self.data_loader = data_loader
    self.label_loader = label_loader
    self.loc_loader = loc_loader
    self.mask_loader = mask_loader
    self.test_data_loader = test_data_loader
    self.test_label_loader = test_label_loader
    self.test_loc_loader = test_loc_loader
    self.test_mask_loader = test_mask_loader
    ...
    self.model_dir = config.model_dir#log_dir
    ...
    print("built  supervisor")
    gpu_options = tf.GPUOptions(allow_growth=True)
    sess_config = tf.ConfigProto(allow_soft_placement=True,
                                 gpu_options=gpu_options)
    print("gpu options")
    #self.sess = sv.PrepareSession(config=sess_config, max_wait_secs=10)
    self.sess = sv.prepare_or_wait_for_session(config=sess_config, max_wait_secs=5)
    #self.sess = tf.Session(config=sess_config)
    print("prepare or wait")

  def train(self):
    losses = np.zeros((self.max_step//self.log_step,))
    acc = np.zeros((self.max_step//self.log_step,))
    ...
    for step in trange(self.start_step, self.max_step):
      ...
      if step % self.log_step == self.log_step - 1:
        ...
        losses[step//self.log_step] = c_loss
        acc[step//self.log_step] = accuracy
  ...
  def build_model(self):
    ...
    self.locs = self.loc_loader
    self.masks = self.mask_loader
    #x = norm_img(self.x)

    self.c_loss, feat, self.accuracy, self.c_var = quick_cnn(
      self.x, self.labels, self.locs, self.masks, self.c_num, self.batch_size, is_train=True, reuse=False)
    ...
    # with tf.variable_scope("C/fc5", reuse=True):
    #   fc4_weights = tf.get_variable("weights")
    # self.fc4 = tf.reduce_max(tf.abs(tf.gradients(self.c_loss, fc4_weights, self.c_loss)))

    x_grad = tf.gradients(self.c_loss, self.x, self.c_loss)
    ...
    self.summary_op = tf.summary.merge([
      ...
      tf.summary.scalar("first layer", self.conv1_grad),
      #tf.summary.scalar("first fc layer", self.fc4),
  ...
  def test(self):
    self.saver.restore(self.sess, self.model_dir+'\model')
    test_accuracy = 0
    for iter in trange(self.test_iter):
      fetch_dict = {"test_accuracy":self.test_accuracy, "feat":self.test_feat}
      result = self.sess.run(fetch_dict)
      test_accuracy += result['test_accuracy']
    test_accuracy /= self.test_iter
  ...
  def build_test_model(self):
    ...
    self.test_loc = self.test_loc_loader
    self.test_mask = self.test_mask_loader
    loss, self.test_feat, self.test_accuracy, var = quick_cnn(
      self.test_x, self.test_labels, self.test_loc, self.test_mask, self.c_num, self.batch_size_test, is_train=False, reuse=True)
